      ...
      <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
          <ul class="navbar-nav ms-auto">
            <li class="nav-item">
              <a class="nav-link pointer-on-hover" @click="closeCard">Home</a>
            </li>
            <slot name="menu"></slot>
            <li class="nav-item">
      ...
      <div class="sidebar bg-black">
        ...
        <ul class="list-group">
          <li class="list-group-item" v-for="category in this.$store.state.categories" :key="category.id">
            <input @click="searchByCat(category.name, category.id)" class="form-check-input me-1 pointer-on-hover"
              type="radio" :name="category.name" :value="category.id" :id="category.id"
              :checked="checkedValue === category.id">
            <label class="form-check-label" :for="category.id"> {{ category.name }} </label>
            <slot name="edit" :categoryId="category.id"></slot>
          </li>
        </ul>
